...
void Canvas::dibujarBordeIzquierdo(SDL_Renderer* renderer) {
		...
		dest.w = 3;
		SDL_SetRenderDrawColor(renderer, COLOR_BORDE_EXTERNO.r, COLOR_BORDE_EXTERNO.g, COLOR_BORDE_EXTERNO.b, 1);
		SDL_RenderFillRect(renderer, &dest);
		dest.x = dest.x + 3;
		dest.y = dest.y + 3;
		dest.h = dest.h - 6;
		dest.w = 2;
		SDL_SetRenderDrawColor(renderer, COLOR_BORDE_INTERNO.r, COLOR_BORDE_INTERNO.g, COLOR_BORDE_INTERNO.b, 1);
		SDL_RenderFillRect(renderer, &dest);
}
